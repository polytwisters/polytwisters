...
def scale_cadquery_workplane(workplane: cadquery.Workplane, x: float, y: float, z: float) -> cadquery.Workplane:
...
def create_cycloplane(w, zenith, azimuth):
    ...
    part = scale_cadquery_workplane(part, scale_x, 1.0, 1.0)
...
def safe_union(part_1, part_2):
    if len(part_1.solids().vals()) == 0:
        return part_2
    elif len(part_2.solids().vals()) == 0:
        return part_1
...
def compute_polytwister_cross_section(polytwister, w, scale=1.0, tolerance=0.1, angular_tolerance=0.1):
    ...
    workplane = scale_cadquery_workplane(workplane, scale, scale, scale)
...
def get_max_distance_from_origin(polytwister, w):
    vertices, __ = compute_polytwister_cross_section(polytwister, w)
    if len(vertices) == 0:
        return 0.0
...
def main():
    ...
    parser.add_argument(
        "out_dir",
        type=str,
        help="Output directory of wavefront OBJ file(s)."
    )
    ...
    if args.w is not None:
        ...
        with open(out_dir / "out.obj", "w") as f:
        ...
        for i in range(num_frames):
            ...
            mesh = compute_polytwister_cross_section(polytwister, w)
            with open(out_dir / f"out_{str(i).rjust(num_digits, '0')}.obj", "w") as f:
                write_mesh_as_obj(mesh, f)
